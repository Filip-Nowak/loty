...
public class MainController {
    ...
    @GetMapping("/search")
    private String showSearchFlightsView(Model model, @RequestParam(name = "from") Optional<String> oFrom, @RequestParam(name = "departure") Optional<String> oDeparture, @RequestParam(name = "to") Optional<String> oTo, @RequestParam(name = "arrive") Optional<String> oArrive, @RequestParam(name = "passengers") Optional<String> oPassengersCount) {
        List<Flight> flights;
        ...
        try {
            ...
            model.addAttribute("flightsArrive", null);
            model.addAttribute("flightsReturn", null);
    ...
    private LocalDateTime getRandomDate() {
        ...
        long startSeconds = LocalTime.MIN.toSecondOfDay();
        long endSeconds = LocalTime.MAX.toSecondOfDay();
        long randomSeconds = ThreadLocalRandom.current().nextLong(startSeconds, endSeconds + 1);
        System.out.println("start:" + startSeconds + "\nend:" + endSeconds + "\nrandom:" + randomSeconds);
